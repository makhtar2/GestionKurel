      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-8 pb-32">
        {view === 'login' && (
          <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-12 animate-in fade-in">
            ...
            <form onSubmit={handleLogin} className="w-full max-w-sm bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl space-y-8 relative overflow-hidden">
              ...
              <button className="w-full py-4 rounded-xl bg-gradient-to-r from-[#dc9b3f] via-[#f0bd53] to-[#f3df8f] text-white font-black uppercase tracking-widest text-[10px] active:scale-95 transition-all shadow-lg shadow-[#f0bd53]/20 relative z-10">
                Se Connecter
              </button>
            </form>
          </div>
        ...
        {view === 'selection' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid gap-3">
              {kourels.map(k => (
                <div key={k.id} onClick={() => { setSelectedKourel(k); loadKourelData(k.id); setView('dashboard'); }} className="p-6 bg-white border border-slate-100 rounded-2xl flex justify-between items-center cursor-pointer hover:border-[#003362] transition-all shadow-sm group relative">
                  ...
                  <div className="bg-[#003362] text-white px-4 py-2 rounded-lg font-black text-sm relative z-10">
                    {kourelStats[k.id]?.rate}%
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#003362] text-white p-8 rounded-[2rem] space-y-6 shadow-xl relative overflow-hidden">
                   ...
                   <div className="grid grid-cols-2 gap-4 relative z-10">
                      <div className="bg-white/5 p-5 rounded-2xl border border-white/10 backdrop-blur-sm relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                   {profile?.role === 'surveillant' && (
                     <button onClick={() => setView('attendance')} className="bg-gradient-to-r from-[#dc9b3f] to-[#f0bd53] text-white p-6 rounded-2xl shadow-xl flex items-center justify-between group active:scale-95 transition-all relative">
                   ...
                   <button onClick={() => setView('history')} className="bg-white border border-slate-100 p-6 rounded-2xl flex items-center justify-between hover:border-[#003362] transition-all shadow-sm relative group">
            ...
            {view === 'attendance' && (
              <div className="space-y-6 pb-20">
                ...
                <div className="space-y-3">
                  {members.map(m => (
                    <div key={m.id} className="bg-white p-4 rounded-xl border border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm relative group">
                      ...
                      <div className="flex gap-1 relative z-10">
                        {[{ l: 'ABSENT', v: 'Absent', c: 'bg-red-600' }, { l: 'NGANT', v: 'Excusé', c: 'bg-[#dc9b3f]' }, { l: 'PRÉSENT', v: 'Présent', c: 'bg-[#003362]' }].map((btn) => (
                          <button key={btn.v} onClick={() => setAttendance({...attendance, [m.id]: btn.v})} className={`px-4 py-2 rounded-lg font-black text-[8px] transition-all relative ${attendance[m.id] === btn.v ? `${btn.c} text-white shadow-lg` : 'bg-slate-50 text-slate-300'}`}>
                ...
                <div className="fixed bottom-24 left-0 right-0 px-6 z-[100]"><button onClick={() => confirmAction("Validation", "Valider la séance ?", saveAttendance)} disabled={saving} className="w-full max-w-xs mx-auto py-4 rounded-xl bg-[#003362] text-white font-black uppercase text-[10px] tracking-widest shadow-2xl block relative">
                  ...
                  {sessionsList.filter(d => d.startsWith(selectedMonth)).map(date => (
                    <div key={date} className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden group">
                       ...
                       {expandedSession === date && (
                         <div className="p-6 bg-slate-50 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-2 relative">
        ...
        {!selectedKourel && ['dashboard', 'attendance', 'history'].includes(view) && (
          <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 text-center space-y-6 animate-in fade-in">
             <div className="bg-slate-50 w-24 h-24 rounded-full flex items-center justify-center shadow-inner relative">
             ...
             <button onClick={() => setView('selection')} className="px-8 py-4 bg-[#003362] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-[#003362]/20 active:scale-95 transition-all relative">
        ...
        {view === 'mgmt' && (
          <div className="space-y-6 animate-in fade-in pb-20">
            <div className="flex bg-slate-100 p-1 rounded-2xl">
              <button onClick={() => setMgmtTab('members')} className={`flex-1 py-3 rounded-xl font-black text-[10px] uppercase transition-all relative ${mgmtTab === 'members' ? 'bg-[#003362] text-white shadow-lg' : 'text-slate-400'}`}>
                <span className="relative z-10">Membres</span>
              </button>
              <button onClick={() => setMgmtTab('access')} className={`flex-1 py-3 rounded-xl font-black text-[10px] uppercase transition-all relative ${mgmtTab === 'access' ? 'bg-[#003362] text-white shadow-lg' : 'text-slate-400'}`}>
            ...
            {mgmtTab === 'members' && (
              selectedKourel ? (
                <div className="space-y-6">
                  <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl space-y-6 relative">
                    ...
                    <div className="space-y-3 relative z-10">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={handleAddOrUpdateMember} disabled={saving} className="flex-[2] py-4 rounded-xl bg-[#003362] text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-[#003362]/20 relative">
                  ...
                  <div className="space-y-2">
                    {allMembers.map(m => (
                      <div key={m.id} className="p-4 bg-white border border-slate-50 rounded-xl flex justify-between items-center group shadow-sm relative">
                ...
                <div className="p-12 text-center space-y-4 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 relative">
                  ...
                  <button onClick={() => setView('selection')} className="px-6 py-3 bg-[#003362] text-white rounded-xl font-black text-[10px] uppercase relative z-10">
            ...
            {mgmtTab === 'access' && (
              <div className="space-y-4">
                {allProfiles.filter(p => p.role === 'visiteur' || profile?.role === 'coordinateur').map(p => (
                  <div key={p.id} className="p-6 bg-white border border-slate-100 rounded-2xl space-y-4 shadow-sm relative group">
